...
// Jason Losh
/*
This library was provided by our Prof. Jason Losh.
I have modified it and added new function to support
TCP/IP state machine for web server.Functions are:
1)sendWebpage
2)sendSynAck
3)sendFinAck
4)sendAck
...
uint32_t sum;
uint8_t macAddress[6] = {1,2,3,4,5,6};
uint8_t ipv4Address[4];
uint32_t rxSeqNumber = 0;
uint32_t txSeqNumber = 0;
uint32_t txAckNumber = 0;
uint32_t dataCount = 0;
uint32_t rxAckNumber = 0;
int getFlag,postFlag,favFlag,fynFlag;
char username[20];
char password[20];
char sensor[3];
// ------------------------------------------------------------------------------
...
struct _tcp	// 20 Bytes
{
	...
	uint16_t destPort;		//2
	uint32_t seqNumber;		//4
	uint32_t ackNumber;		//4
	uint8_t dataOffset;		//1
	uint8_t flags;			//3
	uint16_t windowSize;	//2
	uint16_t checksum;		//2
	uint16_t urgentPtr;		//2
	uint8_t data;			//2
}*tcp;
...
{
	return ((ntohs((value & 0xFFFF0000) >>16)) + ((ntohs(value & 0x0000FFFF)) << 16));
}

#define ntohs htons
#define ntohs32 htonl
...
uint8_t etherIsArp(uint8_t data[])
{
    ...
    ether = (void*)&enc28j60->data;
    arp = (void*)&ether->data;
    ok = (ether->frameType == 0x0608);  // as TI is big Endian and Internet is Little indian so swap the Bytes
    while (ok && (i < 4))
    {
...
uint8_t etherIsTCP(uint8_t data[]){
	...
	uint8_t ok;
	uint16_t tmp_int;
	uint16_t tcpLength,ipDatagramLength;
	enc28j60 = (void*)data;
	ether = (void*)&enc28j60->data;
	ip = (void*)&ether->data;
	...
	if(ok){
		...
		tmp_int = ip->protocol;
		sum += (tmp_int & 0xff) << 8;
		ipDatagramLength = ntohs((ip->length));							//calculating the ipdatagramLength
		tcpLength = ipDatagramLength - ((ip->rev_size & 0xF) * 4);		// subtracting the ipHeader from ipdatagramLength
		tcpLength = ntohs(tcpLength);									// required TCP segment length
		etherSumWords(&tcpLength,2);
		// adding tcpHeader + tcpData
		etherSumWords(tcp, ntohs(tcpLength));
		ok = (getEtherChecksum() == 0);
	}
	return ok;
}

uint8_t etherGetTCPFlags(uint8_t data[]){

	enc28j60 = (void*)data;
	ether = (void*)&enc28j60->data;
	ip = (void*)&ether->data;
	tcp = (void*)((uint8_t*)ip + ((ip->rev_size & 0xF) * 4));
	rxSeqNumber = ntohs32(tcp->seqNumber);
	return tcp->flags;
}
...
uint8_t getPOSTData(char* array, char* data)
{
	int i = 0;
	char* location;
	char name[20];
	location = strstr(array, data);
	if (location != NULL)
	{
		while (*location != '=')
			location++;
...
void processTCPData(uint8_t data[])
{
	uint8_t* tcpData;
	uint16_t i=1;
	enc28j60 = (void*)data;
	...
	tcp = (void*)((uint8_t*)ip + ((ip->rev_size & 0xF) * 4));
	tcpData = &tcp->data;
	getFlag = ((tcpData[0]== 'G') && (tcpData[1]== 'E') && (tcpData[2]== 'T'));
	postFlag = ((tcpData[0]== 'P') && (tcpData[1]== 'O') && (tcpData[2]== 'S')&& (tcpData[3]== 'T'));
	favFlag = ((tcpData[5]== 'f') && (tcpData[6]== 'a') && (tcpData[7]== 'v'));
	rxSeqNumber = tcp->seqNumber;
	rxAckNumber = tcp->ackNumber;
	while(tcpData[i] !=NULL)
		i=i+1;
	dataCount = i;
}


void sendAck(uint8_t data[], uint8_t* tcp_data, uint8_t tcp_data_size)
{
	uint8_t *copy_data;
	uint8_t i, tmp;
	uint16_t tmp_int,tcpLength;
	enc28j60 = (void*)data;
	ether = (void*)&enc28j60->data;
	ip = (void*)&ether->data;
	...
	uint16_t tempPort;
	tempPort = tcp->sourcePort;
	tcp->sourcePort = tcp->destPort ;
	tcp->destPort = tempPort;

	// adjust lengths
	ip->length = ntohs(((ip->rev_size & 0xF) * 4) + 20 + tcp_data_size);
	ip->id = rand();
	...
	// copy data
	copy_data = &tcp->data;
	for (i = 0; i < tcp_data_size; i++)
		copy_data[i] = tcp_data[i];

	tcpLength = ntohs(ip->length)-20;
	tcpLength = ntohs(tcpLength);		// big endian format
	// 32-bit sum over pseudo-header
	sum = 0;
	tcp->checksum=0;
	//adding the puedo header
	etherSumWords(ip->sourceIp, 8);
	tmp_int = ip->protocol;
	sum += (tmp_int & 0xff) << 8;
	etherSumWords(&tcpLength, 2);
	...
	if(!fynFlag)
	{
		tcp->seqNumber = rxAckNumber;
		tcp->ackNumber = ntohs32(ntohs32(rxSeqNumber) + dataCount);
		txAckNumber = tcp->ackNumber;
		txSeqNumber = tcp->seqNumber;
	}
	else
	{
		tcp->seqNumber = rxAckNumber;
		tcp->ackNumber = ntohs32(rxSeqNumber + 1);;
		txAckNumber = tcp->ackNumber;
		txSeqNumber = tcp->seqNumber;
	}

	tcp->flags = ackFlag ;
	// adding the TCP segment now

	etherSumWords(tcp, ntohs(tcpLength));
	tcp->checksum = getEtherChecksum();

//	 send packet with size = ether + tcp hdr + ip header + tcp_data_size
	etherPutPacket((uint8_t*)ether, 34 + ((ip->rev_size & 0xF) * 4) + tcp_data_size);
}

void sendFinAck(uint8_t data[], uint8_t* tcp_data, uint8_t tcp_data_size)
{
	uint8_t *copy_data;
		uint8_t i;
		uint32_t tmp;
		...
		tmp = tcp->seqNumber;
		tcp->seqNumber = tcp->ackNumber;
		tcp->ackNumber = tmp;

		tcp->flags = ackFlag | finFlag;
		// adding the TCP segment now
...
void sendSynAck(uint8_t data[], uint8_t* tcp_data, uint8_t tcp_data_size){
	...
	// adjust lengths
	ip->length = ntohs(((ip->rev_size & 0xF) * 4) + 32 + tcp_data_size);
	ip->id = rand();
	// 32-bit sum over ip header
	sum = 0;
	etherSumWords(&ip->rev_size, 10);
	...
	tcp->seqNumber = rand()+(rand()<<8);
	tcp->ackNumber = ntohs32(rxSeqNumber + 1);
	tcp->flags = ackFlag | synFlag;
...
//	 send packet with size = ether + tcp hdr + ip header + tcp_data_size
	etherPutPacket((uint8_t*)ether, 46 + ((ip->rev_size & 0xF) * 4) + tcp_data_size);
}

void sendWebpage(uint8_t data[], uint8_t* tcp_data, uint16_t tcp_data_size)
{
	//srand(time(NULL));
	uint8_t *copy_data;
	uint16_t tmp_int,tcpLength,i;
	enc28j60 = (void*)data;
	ether = (void*)&enc28j60->data;
	ip = (void*)&ether->data;
	tcp = (void*)((uint8_t*)&ether->data + ((ip->rev_size & 0xF) * 4));
